...
export function registerSPWriteTools(server: McpServer): void {
  server.registerTool(
    'sp_update_campaign',
    {
      description: 'Update a Sponsored Products campaign. Can change name, state, and daily budget.',
    ...
    async (input) => {
      ...
      try {
        ...
        throw new AdsAPIError(`Failed to update SP campaign: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  );
  ...
  server.registerTool(
    'sp_pause_campaign',
    {
      description: 'Pause a Sponsored Products campaign. Shortcut for sp_update_campaign with state=PAUSED.',
    ...
    async (input) => {
      ...
      try {
        ...
        if (error instanceof AdsAPIError) throw error;
        throw new AdsAPIError(`Failed to pause SP campaign: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  );
  ...
  server.registerTool(
    'sp_enable_campaign',
    {
      description: 'Enable a Sponsored Products campaign. Shortcut for sp_update_campaign with state=ENABLED.',
      inputSchema: z.object({
    ...
    async (input) => {
      ...
      try {
        ...
        if (error instanceof AdsAPIError) throw error;
        throw new AdsAPIError(`Failed to enable SP campaign: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  );
  ...
  server.registerTool(
    'sp_update_ad_group',
    {
      description: 'Update a Sponsored Products ad group. Can change name, state, and default bid.',
    ...
    async (input) => {
      ...
      try {
        ...
        if (error instanceof AdsAPIError) throw error;
        throw new AdsAPIError(`Failed to update SP ad group: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  );
  ...
  server.registerTool(
    'sp_update_keyword',
    {
      description: 'Update a Sponsored Products keyword. Can change state and bid.',
    ...
    async (input) => {
      ...
      try {
        ...
        if (error instanceof AdsAPIError) throw error;
        throw new AdsAPIError(`Failed to update SP keyword: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  );
  ...
  server.registerTool(
    ...
    {
      description: 'Update the bid for a Sponsored Products keyword. Shortcut for sp_update_keyword with bid only.',
      inputSchema: z.object({
    ...
    async (input) => {
      ...
      try {
        ...
        if (error instanceof AdsAPIError) throw error;
        throw new AdsAPIError(`Failed to update SP keyword bid: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  );
  ...
  server.registerTool(
    'sp_create_keyword',
    {
      description: 'Create a new Sponsored Products keyword in an ad group.',
    ...
    async (input) => {
      ...
      try {
        ...
        if (error instanceof AdsAPIError) throw error;
        throw new AdsAPIError(`Failed to create SP keyword: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  );
}
