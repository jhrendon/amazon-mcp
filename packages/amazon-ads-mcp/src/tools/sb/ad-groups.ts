...
export function registerSBAdGroupTools(server: McpServer): void {
  server.registerTool(
    'sb_list_ad_groups',
    {
      description: 'List all Sponsored Brands ad groups. Requires Brand Registry.',
    ...
    async (input) => {
      ...
      try {
        ...
        throw new AdsAPIError(`Failed to list SB ad groups: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  );
  ...
  server.registerTool(
    'sb_get_ad_group',
    {
      description: 'Get details for a specific Sponsored Brands ad group by ID. Requires Brand Registry.',
    ...
    async (input) => {
      ...
      try {
        const adGroup = await client.get<AdGroup>(
          `/v2/hs/adGroups/${input.adGroupId}`,
          undefined,
          { rateLimitCategory: 'adGroups' }
        );
        ...
        throw new AdsAPIError(`Failed to get SB ad group ${input.adGroupId}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  );
}
