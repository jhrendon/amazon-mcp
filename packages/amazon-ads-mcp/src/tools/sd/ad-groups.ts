...
export function registerSDAdGroupTools(server: McpServer): void {
  server.registerTool(
    'sd_list_ad_groups',
    {
      description: 'List all Sponsored Display ad groups.',
    ...
    async (input) => {
      ...
      try {
        ...
        throw new AdsAPIError(`Failed to list SD ad groups: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  );
  ...
  server.registerTool(
    'sd_get_ad_group',
    {
      description: 'Get details for a specific Sponsored Display ad group by ID.',
    ...
    async (input) => {
      ...
      try {
        ...
        if (error instanceof AdsAPIError) throw error;
        throw new AdsAPIError(`Failed to get SD ad group ${input.adGroupId}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  );
}
